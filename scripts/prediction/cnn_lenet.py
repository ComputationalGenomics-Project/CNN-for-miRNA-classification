...
def conv_net(params, layers, data, labels):
  ...
  [cost, grad, input_od, percent, outcome] = mlrloss(wb,
                                            output[i-1]['data'],
                                            labels,
                                            layers[i]['num'], 1)
...
def conv_layer_forward(input, layer, param):
  ...
  group = layer['group']
  num = layer['num']
  #print "h_in, w_in: ", h_in, w_in
  #print "num, batchsize: ", num, batch_size
...
def mlrloss(wb, X, y, K, prediction):
  ...
  temp = prob[y, np.arange(batch_size)]
  temp2 = temp.flatten()
  temp_indx = np.where(temp2 == 0)
  if len(temp_indx) != 0:
    temp2[temp_indx] = 1e-11
  temp = temp2.reshape(temp.shape)
  nll = -np.sum(np.log(temp))
  ...
  return nll, g, od, percent, indices
